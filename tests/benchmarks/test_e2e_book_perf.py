...
def test_full_book_100_pages():
    ...
    try:
        ...
        assert elapsed < 30, f"100 pages took {elapsed:.1f}s (threshold 30s)"
    finally:
        os.unlink(pdf)
...
def test_full_book_10_pages():
    ...
    try:
        ...
        assert elapsed < 5, f"10 pages took {elapsed:.1f}s (threshold 5s)"
    finally:
        os.unlink(pdf)
        for f in os.listdir(td):
            os.remove(os.path.join(td, f))
