...
KZOCR 是**中医古籍 OCR 编排工具**：将 kimi OCR（BookPipeline）对 PDF 的识别结果写入 zai 人工校对台（SQLite），导出经人工终校的 Markdown 后推送 kHUB，形成「扫描 → OCR → 校对 → 发布」闭环。当前 main 版本 **v0.20.0**，**v0.7 自适应引擎编排层已全部实现并合入 main**（EngineRegistry / EngineScheduler / GlyphVerifier / Orchestrator + review_manifest 校对反馈闭环 + Celery 生产接线 + §5.5 Box-Guided VL 视觉仲裁）。
...
### v0.7 编排层（已实现，已合入 main）
`run_engine` 现优先委派给 `kzocr/scheduler/` 自适应引擎编排层（受开关控制，旧签名保留为入口）：
```
run_engine()
  → kzocr/scheduler/orchestrator.py: orchestrate_book()
      ...
      └─ review_manifest  校对反馈闭环（§5.6）
  → BookResult → 导出/推送（同 v0.6 路径）
  → Celery 生产链路（celery_tasks/）：process_book_task 可持久化到主线 BookDB（KZOCR_PERSIST_DB=1）
```
...
### v0.6 与 v0.7 的关系（最重要）
- **v0.6（历史基线）**：引擎选择是硬编码 `if-else`（`run_engine → mock/VLM/real`），无注册中心、无调度器、无字形验证调用。此基线仍保留为 `run_engine` 的默认降级路径。
- **v0.7（已实现，已合入 main）**：自适应引擎编排层。方案见 `docs/plans/ocr-engine-unification.v0.7.md`（概览）+ `.v0.7-DETAILED.md`（12 节详设），多角色评审见 `docs/reviews/`。
- 规划的新模块 `kzocr/scheduler/{registry,scheduler,verifier,orchestrator}.py` **已全部实现并落地**（另含 `review_manifest.py` 校对反馈闭环）。`run_engine` 现优先走 v0.7 编排层（受 `KZOCR_ENABLE_CROSS_CHECK` / `KZOCR_ALLOW_CLOUD_VISION` 等开关控制），旧签名保留为委派入口。改动 `run_engine` 等现有入口前，先读 v0.7 概览 §7（迁移策略：委派模式，旧签名保留）。
...
- 评审报告：`docs/reviews/<日期>-round<N>-v0.7/`（多角色）。v0.7 历经多轮评审（round1→round9）后已进入编码并实现、合入 main（当前 v0.20.0）；设计稿中的「待实现」标记已过期，以 `kzocr/scheduler/` 实际代码为准。
