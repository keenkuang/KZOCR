...
**取舍小结**：方案 A 复用度/成本最优，且直接契合现有 custom.db + `humanFinal` 机制；方案 B 体验最佳但成本最高；方案 C 不推荐。建议 **A 为主、按需演进到 B**。
...
## 7. 推荐落地路线（分阶段，便于评审后立项）

- **阶段 0（必须，最小闭环，独立于前端形态单独立项）**：补齐回导入口——CLI `kzocr import` + web 上传路由（§6），含其安全要求与双写权威性规则。**阶段 0 与前端形态 A/B/C 决策无关，应最先落地、最先受益。**
- **阶段 1（推荐，前端形态冻结后再启动）**：方案 A——先抽 `ReviewDataSource` 协议 + `BookDBAdapter`/`CustomDBAdapter` 双适配器（见 §8），再让 `kzocr web` 支持 custom.db 校对模式（校对员视图 + 导出/冻结复用）。**禁止在路由内散点 `if mode==` 判断。**
- **阶段 2（可选）**：PyInstaller 桌面打包（Windows/macOS 纯校对最小依赖包，排除 torch/PaddleOCR/GLM）。
- **阶段 3（演进）**：若校对人员体验要求高，再演进到方案 B 独立前端。
...
1. 前端形态：方案 A（复用 web）/ B（独立前端）/ C（静态导出）？——**评审建议 A 为主、按需演进 B；阶段 0 不依赖此决策。**
2. 是否先做阶段 0（回导入口）单独立项？——**评审建议：是，独立于前端形态先行（已写入 §7）。**
3. 桌面打包是否本期范围（Windows/macOS 支持优先级）？
4. 校对人员"差异高亮 / 原图回溯"增强——**评审建议提升为必需（已写入 §8），非可选**。
