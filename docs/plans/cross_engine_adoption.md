...
5. ✅ cross_divergence 并入 BookDB + Web/REST 暴露（4.4）：HTML 页面 `/book/{code}/divergences` + REST `GET /api/books/{code}/divergences`，含优先级筛选；书籍详情页 `book.html` 子导航「跨引擎分歧」入口（补于 53ff3e2，279df74 提交信息误称已加、实际漏改）。
...
7. ✅ 真实书端到端验证（scripts/e2e_cross_engine_realbook.py）：真实扫描古籍《秘方求真-570》(3 页) 由 PaddleOCR(PP-OCRv6) 与 RapidOCR 两个真实本地 CPU 引擎逐页识别，喂生产级 `run_cross_align` 比对 → 16 分歧(2 high) 落库；Web/REST 暴露(`GET /api/books/{code}/divergences` + `/book/{code}/divergences` 200)与形近字自学习(`井`↔`并` 经 `POST /api/confusion` 学入并 reload 命中)均通过。注：完整 `orchestrate_book` 跨引擎比对挂在 Tier1 vs Tier3(本地 LLM) 失败路径，本机无 GPU/无云端密钥，故 e2e 直接驱动跨引擎校验栈核心（真实引擎+run_cross_align+BookDB+Web/REST+自学习）等价验证。
