...
### 4.3 分歧级视觉仲裁（升级 VisionRecheckAdapter）
当前 `VisionRecheckAdapter.recheck` 对整页发 VL 模型问 PASS/FAIL，粒度粗。
借鉴 ocr_pipeline_v2 `cross_arbitrate.py`：
- 裁出**分歧区**原页图（`Divergence.boxes` → `get_page_image` 原页，非裁切图），只把分歧片段送 VL 模型。
- 裁决映射：`accepted_a` / `accepted_b` / `both_wrong`(→manual) / `uncertain`(→L3)。
- 复用 `AdaptiveTokenBucket` 共享进程级限流（KZOCR 已有 scheduler 限流）。
- 默认关闭，`--enable-arb` 才激活（数据不出域场景不调云端）。
...
2. ⬜ orchestrator 接入：失败路径 Tier1/Tier2 比对 → cross_divergence（4.1）
3. ⬜ 复核队列规则落 HumanGate（4.2，含 conf 门控前置）
