...
2. **深层架构不兼容**：知识模块需要 `db_pg` 提供 `RuntimeDB.create_*_pattern`，原生 psycopg2 连接无此方法——
   即使接通知识模块，仍会崩溃。**已于 2026-07-21 续十二闭环**（见下「已闭环（缺口②）」）。
...
- 缺口 (2) **已于 2026-07-21 续十二闭环**：`book_pipeline` 新增 `_init_runtime_db`（受控开关
  `KZOCR_TCM_KNOWLEDGE`，默认关闭）构造真正的 `RuntimeDB` 实例存为 `self.db_runtime`；`process_book`
  自动发现步骤改为「`db_runtime` 已构造时调三个知识模块 `auto_discover_{herb,meridian,context}_patterns`，否则走
  原 `pipeline/auto_discovery.py` 裸 SQL 桩」。`current_db_book` 经 `BookConnAdapter` 包装满足 `BookDbConn` 契约。
  默认关闭 → 冻结栈行为 100% 不变；开关开启但 PG 不可用 / 构造失败 → 无害降级回裸 SQL 路径。新增
  `tests/test_tcm_ocr_runtime_db_wiring.py`（9 例 + AST 硬守卫）。详见 `CHANGELOG.md` 续十二。
