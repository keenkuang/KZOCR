...
| `run_e2e_batch.py` | 旧版连续古籍扩样批量脚本（2026-07）。**⚠️ 含覆盖式数据丢失 bug**：每次运行覆盖 `summary.json`，07-19 曾冲掉 16 本全量 JSON（不可复算）。已被 `scripts/e2e_expand_books.py`（`--merge` + 每本检查点）取代，勿再使用。 |
