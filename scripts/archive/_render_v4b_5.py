"""渲染 v4b 真过裁 5 页：[83,143,197,227,247]，目视确认是否切墨。

- 绿框=宽块 / 橙框=窄块（cv2 文字行）
- 红框=被 v4b 左界线切中的文字行（lx < v4b_left < rx）→ 即「切墨」
- 红线=v4b 左界  黄线=cut_f 左界  绿线=dl 左界  蓝虚线=正文左缘
输出：crop_compare/abc_overcut/v4b_5/page_XXXX.png
"""
...
OUT = Path("crop_compare/abc_overcut/v4b_5")
...
def main() -> None:
    for n in PAGES:
        ...
        legend = [
            f"p{n:04d} 奇  v4b左={lv4}  cut_f左={lf}  dl_left={dl_left}  正文左缘={body_left}",
            f"分支={br}  过裁(正文)={lv4-body_left}px  被切文字行={n_cut}",
            f"A,B,C,D={A},{B},{C},{D}",
            "绿/橙=块 红框=被v4b切中的文字行 红=v4b 黄=cut_f 绿=dl 蓝虚=正文左缘",
        ]
        y = 6
        for line in legend:
        ...
        pil.save(out)
        print(f"p{n:04d}: v4b左={lv4} 正文左缘={body_left} 过裁={lv4-body_left}px 被切文字行={n_cut} → {out}")
