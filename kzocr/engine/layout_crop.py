...
# 线程安全锁：PP-DocLayoutV3 的底层 C++ 推理对象非线程安全（并发 predict 会触发
# `vector<bool> range_check` 崩溃并降级 cv2，导致并行渲染的输出与串行不一致）。
# 页级并发编排（KZOCR_PAGE_PARALLEL=1）下多个 worker 共享此模块级单例模型，必须用
# 同一把锁串行化推理（含懒加载与 cv2 标定的共享状态），保证并行与串行逐页产出等价。
# 用 RLock：_maybe_calibrate_cv2 内部会再次调用 _doclayout_rect，需可重入。
_DOC_LAYOUT_LOCK = threading.RLock()
...
def _doclayout_rect(
    ...
    """
    # 串行化 PP-DocLayoutV3 推理（含懒加载与共享模型对象），保证并行渲染与串行等价。
    with _DOC_LAYOUT_LOCK:
...
def _maybe_calibrate_cv2(img: np.ndarray, page_num: int, dl_left: int | None = None) -> None:
    ...
    共享标定状态（_cv2_calib* / _cv2_calib_count / _CV2_CALIB_LOCKED）在并行编排下被多
    worker 并发改写，须用 _DOC_LAYOUT_LOCK 保护（且与 _doclayout_rect 同一把 RLock，可重入）。
    """
