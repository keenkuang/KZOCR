"""统一文档导出：从 BookResult 或 zai SQLite 生成 Markdown/JSON。

提取自 ``kzocr/adapter/to_zai_prisma.py``（export_markdown）与
 ``kzocr/export_zai.py``（export_book_markdown / export_json）。
"""
...
def export_markdown(book: BookResult, out_path: Optional[Path] = None) -> Optional[str]:
    """从内存 BookResult 直接导出 Markdown（不经 zai 库）。

    适用场景：刚跑完 OCR、不落 zai 库时的即时导出。
    """
    lines = [
        f"# {book.title}",
        "",
        f"> 来源：{book.publisher}（{book.pub_year}） | "
        f"引擎：{book.engine_label} | book_code：{book.book_code}",
        "",
    ]
    for p in book.pages:
    ...
    for m in book.meridian_patterns:
        lines.append(
            f"- {m.correct_name} ← {m.ocr_error_pattern}"
            f"（{m.entity_type}, {m.meridian_belonging}）"
        )
    lines.append("")
    lines.append("### 语境范式")
    ...
    for t in book.terms:
        lines.append(
            f"- {t.term_name}（{t.sublib}）：{t.error_pattern} → {t.correct_form}"
        )
    lines.append("")
    lines.append("## 方剂")
    for f in book.formulas:
        ...
        for ing in f.ingredients:
            lines.append(
                f"- {ing.herb_name} {ing.dosage_value}{ing.unit or ''}"
                f"（{ing.role_in_formula or ''}）"
            )
        lines.append("")
...
def export_book_markdown(book_code: str, db_path: Optional[str] = None) -> str:
    """从 zai SQLite 库导出最终校正 Markdown（人工终校后）。

    优先 humanFinal，否则 final/consensus。附三大永久范式库沉淀。
    """
    db = db_path or load_config().zai_db
    conn = sqlite3.connect(db)
    try:
        cur = conn.cursor()
        cur.execute("SELECT title, publisher, pubYear FROM Book WHERE bookCode=?",
                     (book_code,))
        row = cur.fetchone()
        if not row:
        ...
        pages = cur.execute(
            "SELECT pageNum FROM Page WHERE bookCode=? ORDER BY pageNum",
            (book_code,),
        ).fetchall()
